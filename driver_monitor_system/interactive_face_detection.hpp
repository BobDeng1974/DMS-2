...
static const char phone_smoke_model_message[] = "Optional. Path to an .xml file with a trained Phone Smoke Detection model.";
...
/// @brief Message for assigning Facial Landmarks Estimation network to device
static const char target_device_message_ps[] = "Optional. Target device for Phone Smoke Detection network (CPU, GPU, FPGA, HDDL, or MYRIAD). " \
"The demo will look for a suitable plugin for device specified.";
...
/// \brief Define parameter for Phone Smoke Detection model file<br>
/// It is an optional parameter
DEFINE_string(m_ps, "", phone_smoke_model_message);
...
/// \brief Define parameter for target device for Phone Smoke Detection network<br>
DEFINE_string(d_ps, "CPU", target_device_message_ps);
