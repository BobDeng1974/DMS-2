...
void FaceDetection::fetchResults() {
    ...
    for (int i = 0; i < maxProposalCount; i++) {
        ...
        // Select the maximum face for phone smoke detection
        if(r.location.area() > area){
            ...
            int new_w = static_cast<int>((bb_enlarge_coefficient + 0.6) * max_size);
            int new_h = static_cast<int>((bb_enlarge_coefficient + 0.6) * max_size);
            maxFace.x = x - static_cast<int>(std::floor(bb_dx_coefficient * new_w / 2));
            maxFace.y = y - static_cast<int>(std::floor((bb_dy_coefficient - 0.2) * new_h / 2));
            maxFace.width = new_w;
...
// PhoneSmoke
PhoneSmokeDetection::PhoneSmokeDetection(const std::string &pathToModel,
                             const std::string &deviceForInference,
                             int maxBatch, bool isBatchDynamic, bool isAsync,
                             double detectionThreshold, bool doRawOutputMessages)
    : BaseDetection("PhoneSmoke Detection", pathToModel, deviceForInference, maxBatch, isBatchDynamic, isAsync, doRawOutputMessages),
      detectionThreshold(detectionThreshold),
      maxProposalCount(0), objectSize(0), enquedFaces(0), width(0), height(0) {
      }

void PhoneSmokeDetection::submitRequest() {
...
void PhoneSmokeDetection::enqueue(const cv::Mat &face) {
    if (!enabled()) return;
    ...
    width = static_cast<float>(face.cols);
    height = static_cast<float>(face.rows);

    Blob::Ptr  inputBlob = request->GetBlob(input);

    matU8ToBlob<uint8_t>(face, inputBlob);

    enquedFaces = 1;
    // std::cout << "enqueue face : " << face.cols << " x " << face.rows << std::endl;
}

CNNNetwork PhoneSmokeDetection::read()  {
    slog::info << "Loading network files for PhoneSmoke Detection" << slog::endl;
    CNNNetReader netReader;
    /** Read network model **/
    netReader.ReadNetwork(pathToModel);
    /** Set batch size to 1 **/
    ...
    /** SSD-based network should have one input and one output **/
    // ---------------------------Check inputs -------------------------------------------------------------
    slog::info << "Checking PhoneSmoke Detection network inputs" << slog::endl;
    InputsDataMap inputInfo(netReader.getNetwork().getInputsInfo());
    if (inputInfo.size() != 1) {
        throw std::logic_error("PhoneSmoke Detection network should have only one input");
    }
    InputInfo::Ptr inputInfoFirst = inputInfo.begin()->second;
    inputInfoFirst->setPrecision(Precision::U8);
    ...
    // ---------------------------Check outputs ------------------------------------------------------------
    slog::info << "Checking PhoneSmoke Detection network outputs" << slog::endl;
    OutputsDataMap outputInfo(netReader.getNetwork().getOutputsInfo());
    if (outputInfo.size() != 1) {
        throw std::logic_error("PhoneSmokece Detection network should have only one output");
    }
    DataPtr& _output = outputInfo.begin()->second;
    output = outputInfo.begin()->first;
    ...
    const CNNLayerPtr outputLayer = netReader.getNetwork().getLayerByName(output.c_str());
    if (outputLayer->type != "DetectionOutput") {
        throw std::logic_error("PhoneSmoke Detection network output layer(" + outputLayer->name +
                               ") should be DetectionOutput, but was " +  outputLayer->type);
    }

    if (outputLayer->params.find("num_classes") == outputLayer->params.end()) {
        throw std::logic_error("PhoneSmoke Detection network output layer (" +
                               output + ") should have num_classes integer attribute");
    }
    ...
    objectSize = outputDims[3];
    if (objectSize != 7) {
        throw std::logic_error("PhoneSmoke Detection network output layer should have 7 as a last dimension");
    }
    if (outputDims.size() != 4) {
        throw std::logic_error("PhoneSmoke Detection network output dimensions not compatible shoulld be 4, but was " +
                               std::to_string(outputDims.size()));
    }
    _output->setPrecision(Precision::FP32);
    _output->setLayout(Layout::NCHW);

    slog::info << "Loading PhoneSmoke Detection model to the "<< deviceForInference << " plugin" << slog::endl;
    input = inputInfo.begin()->first;
    return netReader.getNetwork();
}

void PhoneSmokeDetection::fetchResults() {
    if (!enabled()) return;
    // std::cout << "start fetchResults"<<std::endl;
    results.clear();
    // if (resultsFetched) return;
    // resultsFetched = true;
    const float *detections = request->GetBlob(output)->buffer().as<float *>();
    std::cout << "detections : "<< detections[2] <<std::endl;
