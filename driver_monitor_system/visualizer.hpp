...
// Drawing detected phone or smoke on the frame
class PhoneSmokeVisualizer {
public:
    using Ptr = std::shared_ptr<PhoneSmokeVisualizer>;

    explicit PhoneSmokeVisualizer(int bbThickness = 1,
                                cv::Scalar phoneColor = cv::Scalar(255, 128, 0),
                                cv::Scalar smokeColor = cv::Scalar(0, 255, 128));

    void draw(cv::Mat& img, std::vector<PhoneSmokeDetection::Result> &results);

private:
    int bbThickness;
    cv::Scalar phoneColor;
    cv::Scalar smokeColor;
    float scale;
};
