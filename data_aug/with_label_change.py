...
class change_size(object):
    ...
    def resize(self, image, boxes, size=(300, 300)):
        assert isinstance(image, PIL.JpegImagePlugin.JpegImageFile)
        assert isinstance(boxes, list)
        ...
        h, w, _ = img_shape
        # resize
        resize_scale_x = size[0] / w
        resize_scale_y = size[1] / h
        mode = cv2.INTER_AREA
        if w > 300 or h > 300:
            mode = cv2.INTER_LINEAR
        img = cv2.resize(img, size, mode)
        boxes[:, :4] *= [resize_scale_x, resize_scale_y, resize_scale_x, resize_scale_y]
        x1 = np.maximum(0., boxes[:, 0]).reshape(-1, 1)
        y1 = np.maximum(0., boxes[:, 1]).reshape(-1, 1)
        x2 = np.minimum(float(img.shape[1]), boxes[:, 2]).reshape(-1, 1)
        y2 = np.minimum(float(img.shape[0]), boxes[:, 3]).reshape(-1, 1)
        boxes = np.hstack((x1, y1, x2, y2))

        return Image.fromarray(img.astype(np.uint8)), boxes.astype(np.int64)
