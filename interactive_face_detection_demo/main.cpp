    ...
    try {
        ...
        while (true) {
            ...
            for(auto &&face : prev_detection_results){
                if(static_cast<int>(face.location.area()) > area){
                    crop = face.location & cv::Rect(0, 0, width, height);
                    area = static_cast<int>(face.location.area());
                }
            }
            if(!prev_detection_results.empty()){
                cv::Mat frame = prev_frame(crop);
                sprintf(imgName, "%s%d%s%zu%s", "/home/yang/dataset/new_images/", 23, "_", framesCounter, ".jpg");
